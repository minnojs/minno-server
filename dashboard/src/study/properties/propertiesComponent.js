                    ...
                    [m('p', [
                        ...
                        m('.input-group.space', [
                            ...
                            m('select.c-select.form-control.space',{onchange: e => update_url(e.target.value)}, [
                                ...
                                ctrl.study.versions.length<2 || ctrl.study.versions[ctrl.study.versions.length-2].deploys && ctrl.study.versions[ctrl.study.versions.length-2].deploys.filter(deploy=>deploy.sets.filter(set=>set.status==='running').length>0).length>0 ? '' :
