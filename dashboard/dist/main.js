...
(function () {
    ...
    var downloadChosenFiles = function (study) { return function () {
        ...
        study.downloadFiles(chosenFiles)
            .then(function (url) {
                ...
                a.href=url;
                return downloadLink(url, study.name);})
            .catch(function (err) { return messages.alert({
    ...
    var stimuliGeneratorComponent = {
        controller: function controller(ref){
            ...
            function do_add_stimulus() {
                ...
                var new_stimulus = {
                    stimulus_name:("stimulus_" + (++ctrl.num_of_stimuli)),
    ...
    var restore_dialog$1 = {
        ...
        view: function (ref) {
            ...
            m('.text-xs-right.btn-toolbar',[
                m('a.btn.btn-secondary.btn-sm', {onclick:function (){close(null);}}, 'Close'),
                m('a.btn.btn-primary.btn-sm', {onclick:function (){do_restore$1(ctrl); }}, 'Restore')
            ])
        ]);
    }
    };
    ...
    var do_restore = function (study) { return function (e) {
        e.preventDefault();
        var study_id = study.id;
        var versions = study.versions;
        var close = messages.close;
    ...
    var update_study_description = function (study) { return function (e) {
        ...
        var ask = function () { return messages.confirm({
            ...
            content: {
                view: function view(){
                    return m('div', [
                        m('textarea.form-control',  {placeholder: 'Enter description', value: study_description(), config: focus_it$4, onchange: m.withAttr('value', study_description)}),
                        !error() ? '' : m('p.alert.alert-danger', error())
                    ]);
                }
            }
    ...
    var do_rename = function (study, notifications) { return function (e) {
        ...
        var ask = function () { return messages.confirm({
            ...
            content: {
                view: function view(){
                    return m('div', [
                        m('input.form-control',  {config: focus_it$4, class: 'tmp', placeholder: 'Enter Study Name', value: study_name(), onchange: m.withAttr('value', study_name)}),
                        !error() ? '' : m('p.alert.alert-danger', error())
                    ]);
                }
            }
        ...
        var rename = function () { return rename_study(study.id, study_name)
            ...
            .then(function (){
                var study2 = studyFactory$1(study.id);
    ...
    var do_duplicate= function (study, callback) { return function (e) {
        ...
        var ask = function () { return messages.confirm({
            header:'New Name',
            content: m('div', [
                m('input.form-control', {placeholder: 'Enter Study Name', config: focus_it$4,value: study_name(), onchange: m.withAttr('value', study_name)}),
                !error() ? '' : m('p.alert.alert-danger', error())
            ])
    ...
    var draw_menu$1 = function (study, notifications) { return Object.keys(settings$1)
        .map(function (comp) {
            var config = settings_hash$1[comp].config;
            return !should_display(config, study) 
                ? '' 
                : config.href
                    ? m('a.dropdown-item', { href: config.href+study.id, config: m.route }, [
                        m('i.fa.fa-fw.'+config.class),
                        settings_hash$1[comp].text
                    ])
        ...
        return m('.sidebar-buttons.btn-toolbar', [
            ...
            m('.btn-group.btn-group-sm', [
                dropdown({toggleSelector:'a.btn.btn-secondary.btn-sm.dropdown-menu-right', toggleContent: m('i.fa.fa-bars'), elements: [
                    draw_menu$1(study, notifications)
                ]})
            ]),
            m('.btn-group.btn-group-sm', [
    ...
    var update_config = function (fingerprint, gmail, dbx, server_data) { return fetchJson(params_url(), {
        body: {fingerprint: fingerprint, gmail: gmail, dbx: dbx, server_data: server_data},
        method: 'put'
    ...
    var configComponent = {
        controller: function controller(){
            ...
            function update_server_type_fields(ctrl, fields){
                ...
                var updated = (ctrl.server_data.type() === 'http' && !ctrl.given_conf().server_data.http)
                    ||
                    (ctrl.server_data.type() === 'https' &&
                ...
                m('.container.sharing-page', [
                    ...
                    m('.row', [
                        ...
                        m('.col-sm-8',[
                            ...
                            m('div', m('label.c-input.c-radio', [
                                m('input[type=radio]', {
                                    onclick: function (){ return ctrl.toggle_visibility('fingerprint', true); },
                                    checked: ctrl.fingerprint.enable(),
                            ...
                            m('.form-group.row', [
                                ...
                                m('.col-sm-5', [
                                    m('input.form-control', {
                                        ...
                                        oninput: function (e){ return ctrl.update_dbx_fields(ctrl, {app_key: e.target.value}); },
                                        onchange: function (e){ return ctrl.update_dbx_fields(ctrl, {app_key: e.target.value}); }
                                    })
                                ])
                            ]),
                            m('.form-group.row', [
                                ...
                                m('.col-sm-5', [
                                    m('input.form-control', {
                                        ...
                                        oninput: function (e){ return ctrl.update_dbx_fields(ctrl, {app_secret: e.target.value}); },
                                    ...
                                    m('.form-group.row.space', [
                                        ...
                                        m('.col-sm-5', [
                                            m('textarea.form-control',  {
                                                ...
                                                oninput: function (e){ return ctrl.update_server_type_fields(ctrl, {https:{private_key: e.target.value}}); },
                                                onchange: function (e){ return ctrl.update_server_type_fields(ctrl, {https:{private_key: e.target.value}}); }
                                            })
                                        ])
                                    ]),
                                    m('.form-group.row.space', [
                                        ...
                                        m('.col-sm-5', [
                                            m('textarea.form-control',  {value: ctrl.server_data.https.certificate(),
                                                oninput: function (e){ return ctrl.update_server_type_fields(ctrl, {https:{certificate: e.target.value}}); },
                                                onchange: function (e){ return ctrl.update_server_type_fields(ctrl, {https:{certificate: e.target.value}}); }                                })
                                        ])
                                    ]),
                                    m('.form-group.row.space', [
                                        m('.col-sm-2', [
                                ...
                                m('.form-group.row.space', [
                                    ...
                                    m('.col-sm-6', [
                                        m('input.form-control', {
                                            ...
                                            oninput: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{owner_email: e.target.value}}); },
                                            onchange: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{owner_email: e.target.value}}); }                                })
                                    ])
                                ]),
                                m('.form-group.row.space', [
                                    m('.col-sm-2', [
                                    ...
                                    m('.col-sm-9',
                                        ctrl.server_data.greenlock.domains().map(function (domain, id){ return m('.form-group.row', [
                                                m('.col-sm-8',
                                                    m('input.form-control', {
                                                        type:'input',
                                                        placeholder: ("Domain " + (id+1)),
                                                        value: domain,
                                                        oninput: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{domain: e.target.value, id: id}}); },
                                                        onchange: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{domain: e.target.value, id: id}}); }
                                                    })
                                                ),
                                                m('.col-sm-1',
                                                    m('button.btn.btn-primary', {onclick: function (){ return ctrl.update_server_type_fields(ctrl, {greenlock:{remove:id}}); }},'X')
                                                )
                                            ]); }),
                                        ctrl.server_data.greenlock.domains().some(function (domain){ return domain===''; }) ? '' :
                                            m('.form-group.row', [
                                                m('.col-sm-8',
                                                    m('input.form-control', {
                                                        type:'input',
                                                        ...
                                                        value: '',
                                                        oninput: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{domain: e.target.value, id:-1}}); },
                                                        onchange: function (e){ return ctrl.update_server_type_fields(ctrl, {greenlock:{domain: e.target.value, id:-1}}); }
                                                    })
                                                )
                                            ])
                                    )]),
    ...
    var get_homepage = function () { return fetchJson(homepage_url(), {
        method: 'get'
    }); };

    var update_homepage = function (upper_panel, right_panel) { return fetchJson(homepage_url(), {
        body: {upper_panel: upper_panel, right_panel: right_panel},
        method: 'put'
    ...
    var homepageComponent = {
        controller: function controller(){
            var ctrl = {
                ...
                mode:m.prop('edit'),
                notifications: createNotifications(),
                do_update_homepage: do_update_homepage,
            };
