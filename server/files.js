    ...
    .then(function({study_data, can_write}){
        ...
        .then(files => {
            ...
            return {
                ...
                is_readonly: !can_write || study_data.archive,
                versions: study_data.versions,
                description: study_data.description,
                permission: !study_user ? 'read only' : study_data.archive ? 'archive' : study_user.permission,
...
function delete_files(user_id, study_id, files, server_url) {
    ...
    .then(function({study_data}){
        ...
        return files.map(function(file) {
            const delPath = path.join(config.user_folder,  study_data.folder_name, 'v'+version_id, file);
